...
**Текущий этап: 4 — Browser Automation.** В окне инструментов доступны восемь
...
Текстовая команда остаётся демо; planner, голос и внешние сервисы ещё не подключены.
...
а введённая команда не исполняется. Windows-действия и подтверждение доступны в отдельном окне инструментов. Запись голоса отключена.
...
session/request UUID, режим demo и служебное событие. Микрофон отключён. Запущенные Chrome/VS Code могут выполнять собственные сетевые запросы.
