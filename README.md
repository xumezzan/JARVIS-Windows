...
**Текущий этап: 6 — локальный голос / push-to-talk.** Кнопка «Планировщик команд» открывает
...
Кнопка «Запустить демо» по-прежнему запускает демонстрацию. Добавлен локальный голос; настоящий микрофон/распознавание/звук ещё не проверены. Внешняя отправка не подключена.
