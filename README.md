...
- **Сервисы.** Outlook, календарь Microsoft, Fireflies, Asana, Notion, чаты Teams,
  файлы и книги Excel в OneDrive, локальные файлы и любой MCP-сервер, манифест
  которого владелец прочитал и утвердил.
- **Память.** Профиль меток, граф знаний (один человек между разными сервисами) и то,
  что ассистент выучил из завершённых запусков.
- **Сводка к встрече.** Одна кнопка: календарь, прошлые разговоры, открытые задачи,
  письма от этого человека и его страница — пять чтений, всегда одни и те же, каждая
  строка подписана инструментом и записью сервиса ([docs/MEETING.md](docs/MEETING.md)).
  Модель в этом не участвует; сервис, который лежит, стоит своего раздела, а не сводки.
...
- **Длинная задача.** До 64 шагов и часа работы. Задача идёт как durable-запуск: каждый
  эффект записан в журнал до вызова, поэтому перезапуск процесса её не теряет и ничего
  не повторяет. На экране — чеклист шагов (сделано / идёт / ждёт), счётчик обращений
  к модели и список прерванных задач с кнопкой «продолжить».
...
- **Каналов Teams.** Чаты есть, чтение каналов требует согласия администратора
  тенанта и ждёт этих внешних ворот.
...
## Обновиться и проверить то, что слито

Когда изменение попало в `main`, один запуск подтягивает его и переустанавливает:

```powershell
& '.\scripts\windows\Update-Jarvis.cmd'
```

Сценарий ничего не теряет: он отказывается работать, если вы не на `main`, если в рабочей
копии есть несохранённые правки, если историю нельзя перемотать вперёд или если открыто окно
Jarvis. В каждом из этих случаев он говорит, что не так, и ничего не меняет. Перед установкой
он печатает список того, что нового приехало.

После него запускайте **Jarvis** с рабочего стола или из меню «Пуск» — там будет ровно
то, что в `main`.

При первом запуске открывается экран **«Настройка»**: ключ модели, вход Microsoft,
токены сервисов и состояние голосовой модели — списком, в любом порядке, ничего
обязательного. Дальше он под рукой: «Настройка подключений» на главном экране.
Пароль в Jarvis не вводится: ключи уходят в хранилище Windows, пароль Microsoft
вводится у Microsoft. Подробности — [docs/SETUP.md](docs/SETUP.md).

## Проверки на каждое изменение

Тесты, линтер, форматирование, типы, `pip check`, запуск приложения и сборка пакета
выполняются на **Windows-раннере** для каждого push и каждого pull request
(`.github/workflows/checks.yml`). Зелёная галочка в PR означает, что проверено не только
на машине автора: дефекты этого репозитория — процесс-перенаправитель venv, потолок
переменной окружения, занятый файл, кириллица в пути — больше нигде не воспроизводятся.
...
проверяет зависимости и окно, создаёт **Jarvis** на рабочем столе и в меню «Пуск». Для первой установки нужны
...
микрофон открытым без удержания кнопки — но **пока имя не прозвучало, ничего не пишется и
не распознаётся**. Ожиданием занимается отдельный локальный слушатель с грамматикой из двух
исходов: имя либо «не имя». Он не умеет выдать расшифровку разговора, потому что у него нет
слов, кроме имени; наружу он отдаёт одно «да» или «нет», а звук остаётся внутри убиваемого
вспомогательного процесса и на диск не попадает.

Услышав имя, ассистент включает обычную запись: скажите «Джарвис», короткая пауза, потом
команда — повторять имя во второй раз не нужно. Фраза заканчивается сама на паузе примерно
в секунду. Режим **выключен по умолчанию**, включается только вручную, и отдельная строка
всё время показывает, что делает микрофон: «◉ Жду обращения», «● Идёт запись»,
«◐ Распознавание», «○ Микрофон выключен». Слушатель, который не может работать — нет
модели, недоступно устройство, — выключает режим с сообщением и **никогда не переходит к
записи комнаты вместо этого**. Пока идёт задача, слушатель не взводится.

Ложное срабатывание безопасно по устройству, а не по точности: оно лишь открывает запись,
а всё остальное идёт через тот же планировщик, те же разрешения и те же подтверждения —
дорогое действие по-прежнему требует произнести контрольную деталь.
...
5. Лимиты длинной задачи: 64 действия, 3 уточнения, 60 секунд на ответ провайдера и час
   на задачу, включая ожидание подтверждений, плюс бюджет в 96 обращений к модели. Эти
   потолки доступны только вместе с журналом запуска: без него задача идёт короткой —
   8 действий и 180 секунд. Срок отдельного approval — 60 секунд. Повторов после ошибки
   нет; уже выданные действия не откатываются.
...
- [Asana](docs/ASANA.md): подключение токена, что умеет и чего намеренно не умеет.
- [Notion](docs/NOTION.md): токен, обязательный шаг с расшариванием страниц, границы.
- [Teams](docs/TEAMS.md): отдельное согласие на тот же аккаунт, чаты и почему нет каналов.
- [OneDrive и Excel](docs/ONEDRIVE.md): файлы, листы, запись диапазона и её проверка.
- [Сводка к встрече](docs/MEETING.md): пять чтений, подписанные строки и их границы.
- [Первый запуск](docs/SETUP.md): экран настройки, что без чего работает, про пароли.
- [Главный экран](docs/MAIN_SCREEN.md): четыре зоны, живые данные вместо плиток и три раскладки.
