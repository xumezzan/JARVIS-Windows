...
По решению владельца оболочка перестала быть демонстрацией: строка команд и микрофон
выполняют задачи через PermissionEngine, подтверждение CONFIRM выдаётся автономно по
видимому переключателю, открывается любое установленное приложение, а голос работает в
режиме свободных рук со словом-обращением. Живой OpenAI проверен на Windows; микрофон,
Outlook и приёмка на чистой машине остаются непроверенными.
