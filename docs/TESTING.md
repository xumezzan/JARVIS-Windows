...
Границы задачи задаёт планировщик (`Limits`), а не переменная окружения: короткая
задача — не более 8 шагов, 3 уточнений, 30 с на провайдера и 180 с на задачу; длинная
(`Limits.long()`, окно планировщика по умолчанию) — 64 шага, 60 с на провайдера, час на
задачу и 96 обращений к модели. Превышение даёт статус `limit`, `timeout` или `budget`
и ненулевой код возврата smoke-прогона. Длинные потолки без журнала запуска отклоняются
статусом `error` и категорией `journal_required`.
